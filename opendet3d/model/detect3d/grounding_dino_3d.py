...
class Det3DOut(NamedTuple):
    ...
    boxes3d: list[Tensor]
    scores: list[Tensor]
    geo_qualities: list[Tensor] | None
...
class GroundingDINO3DOut(NamedTuple):
    ...
    all_layers_geo_quality_preds: Tensor
...
class GroundingDINO3D(GroundingDINO):
    ...
    def _forward_train(
        ...
        # 修改点1：训练阶段，返回两个值 (3D输出, 几何质量预测)
        all_layers_outputs_3d, all_layers_geo_quality_preds = self.bbox3d_head(
    ...
    def _forward_test(
        ...
        if isinstance(text_prompts[0], list):
            ...
            for i, captions in enumerate(text_prompts[0]):
                ...
                # 修改点2：测试阶段（chunked text 情况），返回两个值
                all_layers_outputs_3d, all_layers_geo_quality_preds = self.bbox3d_head(
                ...
                geo_quality_preds = all_layers_geo_quality_preds[-1]

                cls_score = cls_scores[0]
                bbox_pred = bbox_preds[0]
                boxes3d_pred = boxes3d_preds[0]
                geo_quality_pred = geo_quality_preds[0]

                det_bboxes, det_scores, det_labels, det_bboxes3d, det_geo_quality = (
                    self.roi2det3d(
                        cls_score,
                        bbox_pred,
                        token_positive_map,
                ...
                geo_qualities.append(det_geo_quality)
            ...
            geo_qualities = [torch.cat([q for q in geo_qualities])]
            ...
            for i, bbox_pred in enumerate(bbox_preds):
                ...
                boxes3d_pred = boxes3d_preds[i]
                geo_quality_pred = geo_quality_preds[i]
